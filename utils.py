...
def setup_sam_3d_body(
    ...
    model, model_cfg = load_sam_3d_body_hf(hf_repo_id, device=device) # check load_sam_3d_body_hf
...
def export_obj_no_gl(mesh_path: str, vertices: np.ndarray, faces: np.ndarray):
    ...
    with open(mesh_path, "w") as f:
        ...
        # faces are usually 0-indexed in numpy; OBJ expects 1-indexed
        for tri in faces:
...
def save_mesh_results(
    ...
    """Save 3D mesh results to files and return OBJ/PLY file paths"""
    import json
    ...
    if outputs:
        ...
        out_path = os.path.join(save_dir, f"{image_name}_outputs.json")
        with open(out_path, "w") as f:
            json.dump(out_data, f, indent=2)
        print(f"Saved output data: {out_path}")

    for pid, person_output in enumerate(outputs):
        mesh_filename = f"{image_name}_mesh_{pid:03d}.obj"
        mesh_path = os.path.join(save_dir, mesh_filename)

        # ---------------------------
        # Try Renderer path first
        # ---------------------------
        try:
            renderer = Renderer(
            ...
            print(f"Saved mesh (Renderer): {mesh_path}")

            # Overlay (requires OpenGL)
            img_mesh_overlay = (
                renderer(
                    person_output["pred_vertices"],
                    person_output["pred_cam_t"],
        ...
        # ---------------------------
            ...
            print(f"Saved mesh (no OpenGL): {mesh_path}")
            print(f"Renderer failed: {e}")

        obj_files.append(mesh_path)

        # ---------------------------
        # BBox image (no OpenGL)
        # ---------------------------
